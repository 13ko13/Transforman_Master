...
namespace
{
	...
	constexpr int graph_height = 32;//画像１枚の高さ
	constexpr float draw_scale = 1.0f;//描画スケール
	constexpr int max_anim_num = 4;//最大アニメーション数
	constexpr int one_anim_frame = 2;//アニメーションの待機フレーム
}

MuzzleEffect::MuzzleEffect(int handle, const Vector2& pos):
	Effect(pos),
	m_handle(handle)
{
	m_animation.Init(
...
void MuzzleEffect::Draw(const std::shared_ptr<Camera>& pCamera)
{
	if (m_isDead) return;
